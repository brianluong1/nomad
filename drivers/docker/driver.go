...
func (d *Driver) createContainerConfig(task *drivers.TaskConfig, driverConfig *TaskConfig,
	...
	if len(task.Resources.NomadResources.Networks) == 0 {
		...
		for _, port := range network.ReservedPorts {
			...
			if nebulaIpAddr != nil {
				publishedPorts[containerPort+"/tcp"] = append(publishedPorts[containerPort+"/tcp"], getPortBinding(*nebulaIpAddr, hostPortStr)[0])
				publishedPorts[containerPort+"/udp"] = append(publishedPorts[containerPort+"/udp"], getPortBinding(*nebulaIpAddr, hostPortStr)[0])
			}
